...
### 추가 수정 (당일)
- **4자리 전용 프롬프트:** 드럼 미터 오른쪽 소수 자리(작은 칸) 무시, 결과 반드시 4자리 명시
- **GitHub 배포 체계 확립:** deploy/ 폴더 별도 git repo → beefree10/seri47 main 브랜치 push
- **PR #1 생성:** v9-1-pr → main (기록용, 배포는 이미 완료)

### 결론
- 동영상 방식 폐기, 사진(카메라/갤러리) 단일화
- 한 달 1회 검침: 8장×12회 = 96장으로 94호 전체 커버
- 실물 미터기 테스트 예정 (다음 달 검침 시)
